...
public class UserPasswordMenu
{
    private static List<Option> Options =
    [
        new Option() { Type = OptionType.TextInputField, Name = "Set jump user password", MinLength = 1, Masked = true },
    ...
    public static string? Show(string? value)
    {
        ...
        while ((keyInfo = Console.ReadKey(true)).Key != ConsoleKey.Enter || Options[index].Type != OptionType.Selection)
        {
            ...
            else if (Options[index].Type != OptionType.Selection && keyInfo.Key == ConsoleKey.Backspace && Options[index].Value.Length > 0)
            {
                ...
                Canvas.WriteFrame(Options[index].InputY, Options[index].InputX - 2, Options[index].Value.Length > 0 ? Options[index].Masked ? "*" : Options[index].Value.Last().ToString() : " ", AnsiColor.Black, Options[index].Value.Length > 0 ? AnsiColor.Grey93 : null);
                    ...
                    (Canvas.Frame.FrameWidth - 4) - Options[index].StartX - 1))
            {
                ...
                Canvas.WriteFrame(Options[index].InputY, Options[index].InputX, Options[index].Masked ? "*" : Options[index].Value.Last().ToString(), AnsiColor.Black, AnsiColor.Grey93);
                Options[index].InputX += 1;
            }
        }
    ...
    private static void Select(ref int index, int newIndex)
    {
        ...
        if (option.Type == OptionType.TextInputField || option.Type == OptionType.NumberInputField)
            Canvas.WriteFrame(option.InputY, option.StartX, (option.Masked ? new string('*', option.Value.Length) : option.Value) + new string(' ', (Canvas.Frame.FrameWidth - 4) - option.InputX - 1),  AnsiColor.Grey85, AnsiColor.Grey19);
        ...
        if (option.Type == OptionType.TextInputField || option.Type == OptionType.NumberInputField)
        {
            Canvas.WriteFrame(option.InputY, option.StartX, (option.Masked ? new string('*', option.Value.Length) : option.Value) + new string(' ', (Canvas.Frame.FrameWidth - 4) - option.InputX - 1), AnsiColor.Black,
                AnsiColor.Grey93);
    ...
    private static void WriteOptions()
    {
        for (var i = 0; i < Options.Count; i++)
        {
            ...
            if (option.Type == OptionType.TextInputField || option.Type == OptionType.NumberInputField)
            {
                ...
                Canvas.WriteFrame(i * 2, 1 + $"{option.Name}: ".Length, (option.Masked ? new string('*', option.Value.Length) : option.Value) + new string(' ', (Canvas.Frame.FrameWidth - 4) - option.InputX - 1), i == 0 ? AnsiColor.Black :  AnsiColor.Grey85, i == 0 ? AnsiColor.Grey93 : AnsiColor.Grey19);
            }
